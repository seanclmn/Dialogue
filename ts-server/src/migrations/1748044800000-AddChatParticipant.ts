...
export class AddChatParticipant1748044800000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. Create the chat_participant table
    ...
    // 3. Drop the old ManyToMany join table
    await queryRunner.dropTable('user_chats_chat');
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // 1. Re-create the old ManyToMany join table
    await queryRunner.createTable(
      new Table({
    ...
    // 2. Drop the chat_participant table (FKs drop automatically)
